...
# PATCH_WIDTH = 100
# PATCH_HEIGHT = 100
PATCH_WIDTH = 64    # edited by quindex
PATCH_HEIGHT = 64   # edited by quindex
...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    phone_image = tf.reshape(phone_, [-1, PATCH_HEIGHT, PATCH_WIDTH, 3])    #quindex: seperate an image into several blocks by patch-size

    dslr_ = tf.placeholder(tf.float32, [None, PATCH_SIZE])
    dslr_image = tf.reshape(dslr_, [-1, PATCH_HEIGHT, PATCH_WIDTH, 3])  #quindex: seperate an image into several blocks by patch-size

    adv_ = tf.placeholder(tf.float32, [None, 1])    #quindex: combination rate between enhanced_gray and dslr_gray
    ...
    loss_color = tf.reduce_sum(tf.pow(dslr_image - enhanced, 2)) / (2 * batch_size) #edited by quindex
    ...
    all_zeros = np.zeros((batch_size, 1))   #edited by quindex
    test_crops = test_data[np.random.randint(0, TEST_SIZE, 5), :]

    logs = open('models/model.txt', "w+")
    logs.close()

    for i in range(num_train_iters):

        if i == 1000:   #edited by quindex
        ...
        if i % eval_step == 0:
            ...
            for crop in enhanced_crops:
                ...
                misc.imsave('results/' + 'model' + "_" + str(idx) + '_iteration_' + str(i) + '.png', before_after)
                idx += 1
            ...
            saver.save(sess, 'models/' + 'model' + '_iteration_' + str(i) + '.ckpt', write_meta_graph=False)
