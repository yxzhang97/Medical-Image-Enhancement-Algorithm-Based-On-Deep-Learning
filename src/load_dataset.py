...
def load_test_data(phone, dped_dir, IMAGE_SIZE):
    ...
    test_directory_phone = dped_dir + '/training/training_data/trainingdata/'
    test_directory_dslr = dped_dir + '/training/training_data/groundtruth/'
    ...
    for i in range(0, NUM_TEST_IMAGES):
        ...
        I = np.asarray(misc.imread(test_directory_phone + str(i) + '.png')) #edited by quindex
        I = np.float16(np.reshape(I, [1, IMAGE_SIZE]))/255
        test_data[i, :] = I
        
        # I = np.asarray(misc.imread(test_directory_dslr + str(i) + '.jpg'))
        I = np.asarray(misc.imread(test_directory_dslr + str(i) + '.png'))  #edited by quindex
        I = np.float16(np.reshape(I, [1, IMAGE_SIZE]))/255
        test_answ[i, :] = I
...
def load_batch(phone, dped_dir, TRAIN_SIZE, IMAGE_SIZE):
    ...
    train_directory_phone = dped_dir + '/training/training_data/trainingdata/'
    train_directory_dslr = dped_dir + '/training/training_data/groundtruth/'
    ...
    for img in TRAIN_IMAGES:
        ...
        I = np.asarray(misc.imread(train_directory_phone + str(img) + '.png'))  #edited by quindex
        I = np.float16(np.reshape(I, [1, IMAGE_SIZE])) / 255
        train_data[i, :] = I

        # I = np.asarray(misc.imread(train_directory_dslr + str(img) + '.jpg'))
        I = np.asarray(misc.imread(train_directory_dslr + str(img) + '.png'))   #edited by quindex
        I = np.float16(np.reshape(I, [1, IMAGE_SIZE])) / 255
        train_answ[i, :] = I
